...
from app_state import chat_by_id, chat_messages, now_stamp, selected_chat
from messages_view import MessagesRefs, build_messages_list_container, render_messages
from sidebar import SidebarRefs, render_chat_lists
from welcome_ui import build_welcome_view
...
def build_chat_content(
    *,
    sidebar_refs: SidebarRefs,
    build_composer_row: Callable[[Callable[[str], None]], None],
) -> ChatLayoutRefs:
    ...
    welcome_view = build_welcome_view(
        title='Welcome',
        subtitle='We missed you, Rachel!',
    )

    chat_view = ui.column().classes('w-full h-full min-h-0 overflow-hidden hidden')
    with chat_view:
